...
def generateBinarySegmentationMasks(
    ...
    # NOTE (vendored patch): only directories whose name starts with "BraTS" — avoids stray
    # files (e.g. BraTS-MET's BraTS-2026_Inst-Seg.json) being treated as cases.
    relevantFolders = sorted([x.name for x in inputFolderRoot.glob("*")
                              if x.is_dir() and x.name.startswith("BraTS")])
